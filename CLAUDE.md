...
- **Terminal-emulator backgrounds can't safely use any `surface.*` tier — and tinting with the accent doesn't fix it either.** Every surface tier (`bg_sunk`, `bg_soft`, `bg_overlay`) is bit-identical to some `ansi.*` foreground color on at least one flavor — the foundation's neutral ramp is shared between surfaces and ANSI grays by design. `bg_sunk` collides with `ansi.black` on `midnight`/`twilight` and `ansi.bright_white` (used constantly as plain terminal text, not just reverse-video) on `noon`; `bg_soft`/`bg_overlay` collides with `ansi.bright_black` on `midnight` and `ansi.bright_white` on `dawn`. The obvious next idea — blend the variant's accent color into a surface tier so `terminal.background` isn't purely neutral — was checked numerically (all 4 flavors × 6 variants × alpha 0–100%) and doesn't work: contrast against the nearest ANSI color never exceeds ~1.6:1 at any blend strength (AA needs 4.5:1), because the problem is luminance-packing, not hue — the ramp only has ~4 distinct lightness steps, both surfaces and ANSI grays are drawn from it, and a light tint barely moves luminance. Push the tint strong enough to actually separate the luminance and contrast against `terminal.foreground` collapses instead (down to ~4:1 well before the ANSI problem is fixed). Worse: this isn't new to this PR — the original, untouched `terminal.background = surface.bg` on `main` already sits at ~1.0–1.7:1 contrast against some neutral ANSI color on every flavor; nobody had checked contrast ratios here before, only exact-hex equality. No token substitution or port-side blend fixes this — `terminal.background` is intentionally left equal to `surface.bg` (matching `editor.background`) pending a foundation-level fix: the neutral ramp needs more distinct luminance steps, or ANSI colors need to be decoupled from the surface ramp entirely. File that upstream against `vivid-life-design-system` before changing this mapping, and re-verify contrast ratios (not just exact-match) against every `ansi.*` value per flavor if you do.
