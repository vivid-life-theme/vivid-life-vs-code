...
- **Terminal-emulator backgrounds can't safely use any `surface.*` tier.** Every surface tier (`bg_sunk`, `bg_soft`, `bg_overlay`) is bit-identical to some `ansi.*` foreground color on at least one flavor — the foundation's neutral ramp is shared between surfaces and ANSI grays by design. `bg_sunk` collides with `ansi.black` on `midnight`/`twilight` and `ansi.bright_white` (used constantly as plain terminal text, not just reverse-video) on `noon`; `bg_soft`/`bg_overlay` collides with `ansi.bright_black` on `midnight` and `ansi.bright_white` on `dawn`. No token substitution in this file fixes it — `terminal.background` is intentionally left equal to `surface.bg` (matching `editor.background`) pending a foundation-level fix (e.g. a dedicated terminal-surface token, or de-coupling ANSI colors from the surface ramp). File that upstream against `vivid-life-design-system` before changing this mapping, and re-verify against every `ansi.*` value per flavor if you do.
