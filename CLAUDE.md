...
- **Content surfaces vs. chrome surfaces.** Every port needs two visually distinct backgrounds: `surface.bg` for panes the user edits/reads content in (editor, active tab, peek view, notebook), and `surface.bg_sunk` for panes that just frame them (sidebar, activity bar, status bar, title bar, tab strip, **and the panel/terminal container**). Mapping a framing pane to `surface.bg` — matching the content it's supposed to visually separate from — is the recurring bug this system is prone to; check for it in every new port. Note: on `midnight`/`twilight` (dark flavors), `surface.bg_sunk` is bit-identical to `ansi.black` in the foundation tokens, so a terminal-emulator pane using `bg_sunk` as its background makes literal ANSI-black foreground text unreadable there. This matches how several well-known terminal color schemes (One Dark, Dracula) treat ANSI black — as a reverse-video block color, not body text — so it's an accepted tradeoff here, but it's a real AA-contrast failure (1:1) worth raising upstream if a port needs it fixed at the source (e.g. a dedicated terminal-surface token, or de-coupling `ansi.black` from `bg_sunk`).
